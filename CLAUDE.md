...
| `appraisal_schema.py`       | `AppraisalSchema`, `AppraisalDimension`, `SCHERER_CPM_SCHEMA` (default), `DIRECT_VAD_SCHEMA` (opt-in, LLM rates V/A/D directly — stronger human-gold agreement, Addendum V; yields `GenericAppraisalVector` so dual-path `elaborate()`/SEC features need the default) — pluggable appraisal-theory schemas (OCC, GRID, custom) |
...
| `logging_config.py`         | `configure_logging()` — package logger setup (level from arg or `EMOTIONAL_MEMORY_LOG_LEVEL`; optional JSON)                                                                                                                                                                                                                   |
...
**retrieve**: `embed query → build_retrieval_plan() (pure, no side-effects) → Pass 1 (6-signal score, no spreading) → seed set → spreading_activation() (BFS multi-hop) → Pass 2 (activation_map boost) → per-memory: compute_ape() + update_prediction() → APE-gated reconsolidation → hebbian_strengthen() on co-retrieved links → return top-k`. Use `retrieve_with_explanations()` to expose `RetrievalExplanation` / `RetrievalBreakdown` / `RetrievalSignals` per result. By default the affect-proximity signal (s3) uses the runtime affective state; pass the keyword-only `query_affect: CoreAffect` to `retrieve()`/`retrieve_with_explanations()` to score against the query's own affect instead (no state mutation), or call `retrieve_with_query_appraisal()` to appraise the query text and use that affect — the production-reachable retrieve-time query-appraisal path (Addendum T), best paired with `DIRECT_VAD_SCHEMA` — bounded to the affect-discriminative regime (Addendum T2A found it does not beat cosine on naturalistic dialogue). `retrieve_query_gated()` (Addendum Y, v0.17.0) is the safe wrapper: `|valence| < query_affect_gate_tau` (default 0.2) routes to semantic-only weights, else to the query-appraisal path.
...
- **Config-driven**: All behavior parameterized via nested config classes (`EmotionalMemoryConfig`, `DecayConfig`, `RetrievalConfig`, `ResonanceConfig`, `MoodDecayConfig`, `AdaptiveWeightsConfig`, `LLMAppraisalConfig`, `QueryClassifierConfig`). Top-level flags on `EmotionalMemoryConfig`: `dual_path_encoding` (bool), `elaboration_learning_rate` (float, blend ratio in `elaborate()`), `auto_categorize` (bool, run Plutchik categorization on encode), `appraisal_max_concurrency` (int, default 8, `ge=1` — bounds parallel appraisal in `encode_batch`; set to 1 for fully sequential / a non-thread-safe `llm` callable), `max_content_length` (`int | None`, reject oversized `encode`/`retrieve` strings), `query_affect_gate_tau` (float, default 0.2 — neutral-query threshold for `retrieve_query_gated()`). Ablation toggles: `enable_appraisal`, `enable_mood_signal`, `enable_momentum`, `enable_resonance`, `enable_reconsolidation` (all `bool = True`).
