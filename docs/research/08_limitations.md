...
0.133, Δ=−0.150 [−0.165, −0.136], p_one=1.0000, d=−0.622; MDE 0.018), consistent across
...
**Update (Addendum Y, 2026-07-07) — a query-affect gate removes the self-inflicted penalty, not the boundary.** Gating the affect channel on the appraised query (neutral query -> pure cosine, else the affect-conditioned arm) is a validated safe wrapper (Branch A: Hg1 recover + Hg2 preserve both PASS): it recovers exactly the neutral-query component of the off-regime penalty (~50% on ES-MemEval’s 45.5%-neutral queries; 0% on MADial, where every distressed-user query is affect-carrying) and preserves the full on-regime gain (+0.095 vs cosine on curated). It does **not** move the X/X2 boundary: affect-carrying queries whose gold is counter-congruent (X) or content-determined (X2) stay un-gateable. Shipped as `retrieve_query_gated()` in v0.17.0 (`EmotionalMemoryConfig.query_affect_gate_tau`, default 0.2). See `benchmarks/preregistration_addendum_y_query_affect_gate_closure.md`.


**Update (Addendum Z, 2026-07-14) — learned linear weights do not generalize on third-party gold.** The last untested lever — a held-out linear learning-to-rank profile over the 6 AFT retrieval signals (k=5 cross-fitting, Holm m=3) — **does not** beat cosine on any third-party corpus (MADial Δ=+0.015 p_holm=0.314; ES-MemEval Δ=−0.015 p_holm=0.9997; DailyDialog Δ=+0.008 p_holm=0.811; Branch B). The fixed weight vector was mis-specified (learned beats fixed by +0.153/+0.186 on MADial/ES-MemEval), but optimal held-out linear weighting is **not sufficient** to close the cosine gap where gold is counter-congruent (X) or affect-orthogonal (X2). On curated, learning preserves and amplifies the on-regime advantage (learned top1=0.425 vs fixed 0.240, Hz2 PASS). Negative learned s2 on MADial/ES-MemEval matches the counter-congruent direction but does not translate into a win. The boundary upgrades from "these fixed weights fail" to "no held-out learned linear affect profile generalizes out-of-sample on third-party corpora." See `benchmarks/preregistration_addendum_z_learned_profile_closure.md`.
