...
| `locomo_external_qa_negative` | External benchmarks | Controlled evidence | `4_realistic_tasks` | On the LoCoMo conversational QA benchmark (1986 QA pairs), AFT retrieval underperforms a naive RAG baseline (F1 0.168 vs 0.271; Gate 1 not met). | Pre-registered S1 run completed 2026-04-27. H1 and H2 both fail (Δ<0, p_one=1.0). Add. J Pareto sweep (2026-05-06): 10 weight configs × 200-QA subsample, Hj1 FAIL — no config matches naive_rag on any category. Best: W2 aggregate F1=0.1765 vs naive_rag=0.2092. Per-task base_weights tuning line closed. **Addendum L (2026-05-19)**: query-type routing benchmark in progress — Hl1 confirmatory (routing vs W2), Hl2 secondary (routing vs naive_rag), Hl3 exploratory (classifier accuracy). | Whether closed-loop query-type routing closes the aggregate gap. Whether selective affect suppression or factual/affective dual branching could close the gap. | Addendum L closure; if Branch B, architectural approaches remain open. |
...
- **Addendum L (query-type routing, 2026-05-19)**: closed-loop benchmark in progress — tests whether heuristic query-type routing (derived from Addendum J Pareto closure) improves aggregate F1 vs W2 baseline (Hl1) or closes gap vs naive_rag (Hl2). 200-QA smoke test executing; full 1 540-QA confirmatory run to follow.
