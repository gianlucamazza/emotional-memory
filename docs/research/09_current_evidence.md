...
| `query_affect_gate`                  | Retrieval mechanism        | Controlled evidence          | `4_realistic_tasks`      | Addendum Y (Branch A, 2026-07-07): a query-affect-conditioned gate — route the query to pure cosine when the appraised query is affectively neutral (|valence|<0.2), else to the retrieve-time affect-conditioned arm — is a validated partial safe wrapper. Hg1 (recover, ES-MemEval) and Hg2 (preserve, realistic_recall_v2) both PASS (Holm m=2, gated>aft +0.076 and gated>cosine +0.095).                                                                                                                                                                                                                                                                                                                                                                      | Addendum Y executed 2026-07-07 (4 corpora, tau=0.2 primary, Holm m=2). Hg1 (ES-MemEval) gated>aft delta=+0.076 p_holm=0.0000; Hg2 (realistic_recall_v2) gated>cosine delta=+0.095 p_holm=0.0000. Per-corpus recovery tracks the neutral-query fraction: ES-MemEval 45.5% neutral → 50% recovered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | A production retrieve_query_gated() src API (Branch-A follow-up); gains beyond safe-wrapper (the gate cannot beat cosine on affect-orthogonal/counter-congruent corpora).                                                                                                                       | Promote the gate to a production API in its own pre-registered PR.                                                                       |
