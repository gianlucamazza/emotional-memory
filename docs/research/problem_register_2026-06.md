...
solvable in the short term, the correct resolution is to _bound or correct the
claims_ rather than fake a fix. Technical issues that are fixable are listed with
...
left is to make sure no _public_ surface drifts ahead of that record, plus two
...
and the leakage-free re-run with the _recalibrated_ mapping is significantly
...
r≈0.79–0.83) acts as a _net distractor_ in a regime where semantics already
...
contrast that must _not_ be inflated: dual-path scheduling beats synchronous
appraisal (Hp3, d≈0.95) — that is a _within-AFT_ result, not a win over cosine.

### A2 — The headline advantage requires oracle affect _and_ query-time state injection

**Problem.** The strongest PASSes — realistic*recall_v2 (SBERT Δ=+0.205 [0.150,
0.265], d=0.49; e5 Δ=+0.155, d=0.31) and French (me5 Δ=+0.18 [0.11, 0.26],
g=0.424) — all inject preset valence/arousal at encode time \_and* align the
...
**Root cause.** AFT retrieval signals are _state-based_ and the query is never
...
**Correct resolution.** Every _public_ citation of the +0.21 / +0.18 numbers must
...
**Root cause.** The advantage is regime-specific to _affect-discriminative_
...
**Problem.** Appraisal is _diagnosed_ (Addendum N: valence Pearson r=0.883 vs
gold but +0.169 bias; prompt recalibration Hn1/Hn2 FAIL) and _partly fixed_
...
### A7 — Single-seed; cross-run variance uncharacterized — _Characterized (near-deterministic)_
...
and the reported CIs are bootstrap CIs _within_ a single run, not across seeds.
...
runner, and reports cross-seed mean/stdev/min/max.

**Finding — near-deterministic, _not_ exactly deterministic (corrected).** An
earlier version of this entry claimed "cross-seed stdev = spread = 0.0000 —
per-query outcomes identical across seeds, retrieval verified deterministic."
A fresh validation sweep falsified that headline: the RNG seed itself moves
nothing, but the result is **not bit-stable across runs**. Repeating
`make bench-multiseed` six times gave `retrieval_deterministic=True` in 5/6 runs
and `False` in 1/6 (cross-seed stdev 0.0024), and the absolute `aft` mean drifted
across sweeps (0.120–0.125). The cause is the timing effect already documented:
the engine stamps encode/retrieve with real wall-clock time (`datetime.now`) and
ACT-R decay tracks `now − encoded_at`; because each seed's subprocess is launched
at a slightly different instant, a query at a numerical tie can flip **even with
subprocess isolation** — the isolation removes RNG coupling but not wall-clock
timing. This is correct production behaviour (decay should track real time), not a
defect: the variance is sub-CI (inside the reported bootstrap CIs) and does not
change the scientific conclusion — AFT still clears `naive_cosine` by far more than
the spread (Δ ≈ +0.075). The committed `multiseed_results.md` now carries a standing
caveat to this effect. Threading an injected clock through the core API to make the
benchmark time-deterministic was again judged disproportionate for a sub-CI,
benchmark-only effect. Correct scope: **near-deterministic; sub-CI, timing-driven
variance on near-ties.** See [`08_limitations.md`](08_limitations.md) §2.9.

---

## 3. B — Response-doc self-corrections

### B1 — "Already addressed" conflates _scoped_ with _solved_
...
For §3.1 this is misleading: the _criticism_ — no downstream win — is not solved;
...
relabel §3.1 to _Honestly scoped, not solved_. (§3.3 is legitimately resolved by
_under_-claiming, which is a real resolution; it keeps "Resolved" with a note.)
...
### C2 — Bare "Emotional memory" one-liners _(optional, low)_
...
without scope. This is a metaphor that _could_ imply downstream benefit.
...
**with one correction**: the template's "requires oracle affect _or accurate
appraisal_" is itself an overclaim (Addendum P shows recalibrated appraisal still
...
existing `SECURITY.md` _Scope_ section (dev/optional/third-party deps are out of
