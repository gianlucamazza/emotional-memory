...
- Δ (appraised − cosine) = **−0.164** [−0.179, −0.149], p_one = 1.0000, Cohen's d = −0.653
- u_nDCG@4: cosine **0.284** vs AFT **0.120** (standard-formula nDCG@4: 0.528 vs 0.229);
  ...
  (Δ from −0.152 to −0.183), and is worst at rank 1 (nDCG@1 0.378 vs 0.142)
- Zero-gold-inclusive variant (N=1,427): u_nDCG@4 0.225 vs 0.095 — same picture
- **Power:** MDE at 80% power = 0.019 ≪ observed |Δ| = 0.164 → a _powered negative_ by
...
- **D1 (appraisal fidelity vs third-party labels): AUC = 0.971** [0.938, 0.994]
  (positive vs negative session labels, n=7/376; mean appraised valence +0.700 vs
...
- **D2 (corpus affect-discriminativeness): 68.2%** of queries have a gold-set mean
...
2. **Provenance** (X + X2): positive evidence exists only on author-crafted corpora.
   ...
   (Δ=−0.083 and Δ=−0.164), each in an affect-discriminative regime (D2 76.9% / 68.2%),
   each with faithful appraisal (D1 0.996 / 0.971).
