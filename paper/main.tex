...
\newcommand{\zenodoversiondoi}{10.5281/zenodo.20475352}% [ssot:version_doi]
...
LLM memory systems retrieve primarily by semantic similarity and recency,
neglecting the affective modulation of human memory well-documented in cognitive
psychology. AFT encodes every memory event with a five-component affective
fingerprint: a Russell circumplex core affect, affective momentum (velocity and
acceleration of emotional change), a slow-adapting PAD mood field, a Scherer
appraisal vector, and a resonance graph with spreading activation and Hebbian
strengthening. Retrieval uses a six-signal composite scorer with mood-adaptive
weights, and a Pearce-Hall prediction-error gate controls reconsolidation
lability. We validate the model through 127 parametrized fidelity test cases
covering 20 psychological phenomena, and report a comparative benchmark against a
semantic-only baseline on a new affect-labeled dataset of 258 examples. On a
replayable multi-session realistic recall benchmark (v2, $N=200$, 5 challenge
types), AFT achieves \texttt{top1\_accuracy}=0.53 vs.\ 0.33 for semantic-only
cosine retrieval ($\Delta=+0.21$, 95\,\% CI [0.15,\,0.27], $p<0.001$, $d=0.49$;
SBERT bge-small-en-v1.5), replicated on a second embedder (e5-small-v2:
$\Delta=+0.16$, $p<0.001$). The advantage holds under oracle-provided affect
labels; with \texttt{LLMAppraisalEngine} in dual-path mode on affect-free data
(Addendum~G, $N=200$), architecture parity is maintained but the pre-registered
minimum effect is not reached (Hg1 FAIL, $\Delta=-0.010$, $d=-0.032$); a
recalibrated appraisal mapping re-tested on a leakage-free set reaffirms this
boundary more sharply (Addendum~P, $N=160$: $\Delta=-0.087$, $p=0.002$), scoping
the main result to preset-affect scenarios; a pre-registered gating study
(Addendum~Q, $N=200$ mixed-gate queries) closes the affect-routing line---gating
the affect channel per query recovers the always-on penalty exactly, yet neither
gated nor always-on AFT exceeds cosine, locating the boundary at retrieve-time
state injection rather than appraisal calibration. AFT encodes 25$\times$ faster than
Mem0 (45\,ms vs.\ 1130\,ms per item, CPU-only) with no runtime LLM call at
retrieval. The implementation is released as the open-source MIT-licensed
...
line is closed; the residual hypothesis, retrieve-time query appraisal as a new
retrieval signal, is left unscheduled.
...
(Addenda A--P, plus the Hd2 power top-up) since initial submission. The expanded realistic-recall benchmark ($N=200$, five
...
beyond the English/Italian/Spanish/French slices validated here; retrieve-time
query appraisal as a candidate new retrieval signal (per-category weight routing,
query-type classification, and per-query affect gating are now closed negatives,
Addenda~J/L/Q);
