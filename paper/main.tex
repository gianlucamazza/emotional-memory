...
preset-affect scenarios. AFT encodes 25$\times$ faster than Mem0
(45\,ms vs.\ 1130\,ms per item, CPU-only) with no runtime LLM call at
retrieval, making it practical for latency-sensitive agents. The
implementation is released as the open-source \texttt{emotional-memory}
...
\begin{enumerate}
  ...
  \item 126 parametrized fidelity test cases validating 20 psychological phenomena (\S\ref{sec:fidelity}; intra-theory coherence validation — per-layer retrieval attribution is below detectable effect size at $N=200$, see \S\ref{sec:limitations}).
...
A staged evidence programme has produced eleven pre-registered studies
(Addenda A--K) since initial submission. The expanded realistic-recall benchmark ($N=200$, five
