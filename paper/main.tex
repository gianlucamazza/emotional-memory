...
\newcommand{\zenodoconceptdoi}{10.5281/zenodo.19972284}% [ssot:concept_doi]
\newcommand{\zenodoversiondoi}{10.5281/zenodo.20021634}% [ssot:version_doi]
...
All benchmarks, appraisal prompts, and scenario corpora are English-only.
Performance on other languages is untested.
...
A multilingual Italian slice (G6, $N=80$)\footnote{Addendum~H companion writeup:
...
$\Delta=+0.15$ ($p=0.0005$) with English-centric SBERT bge-small-en-v1.5,
and $\Delta=+0.16$ ($p=0.001$) with multilingual-e5-small (intfloat,
117M parameters, 100+ languages).
Switching to the multilingual embedder raises naive\_cosine top1 from 0.15 to
0.21 (+40\%), confirming that the absolute-accuracy gap vs the English v2
baseline ($\text{top1}=0.53$) was driven primarily by the English-centric
embedder rather than by AFT itself.
...
A pre-registered ablation programme (Addendum~E)\footnote{Pre-registration:
...
results:
\href{\repourl/blob/main/benchmarks/ablation/results.sbert.md}{\texttt{ablation/results.sbert.md}}.}
tested the load-bearing contribution of each AFT component on the
realistic-recall benchmark ($N=100$ queries, v1.4, SBERT bge-small-en-v1.5).
Removing individual layers (mood, resonance, appraisal, momentum, and
APE-gated reconsolidation) produces no measurable change in
top1\_accuracy ($|\Delta|\leq 0.01$, $p_{\mathrm{adj}}=1.000$ under
Holm-Bonferroni across the five comparisons): on this benchmark the
layers are jointly sufficient but not individually load-bearing, and
$H_{e2}$ (reconsolidation) is null. A symmetric dual-path encoding variant
(70/30 blend of appraised and raw core affect, with the
\texttt{KeywordAppraisalEngine}) is destructive
($\textit{top1}=0.35$, $\Delta=-0.35$, $p_{\mathrm{adj}}<0.001$;
$H_{e1}$ rejected). The destructive component is the keyword appraisal
itself: synchronous keyword appraisal collapses retrieval further
($\textit{top1}=0.07$), whereas deferring it via the dual-path slow
elaboration partially recovers signal ($H_{f1}$ PASS,
$\Delta_{\mathrm{dual\,vs\,sync}}=+0.28$). Dual-path encoding is therefore
retained as an opt-in feature only; semantically-grounded LLM appraisal
(non-keyword) is recommended when an appraisal engine is configured.
...
A staged evidence programme has produced four pre-registered results since
...
(protocol in \texttt{benchmarks/human\_eval/}); multilingual coverage beyond
the Italian proof-of-concept (G6); and per-task retrieval weight tuning to
investigate the LoCoMo scope limitation. A dominance dimension is scoped for
v0.8.0 following \texttt{docs/research/11\_dominance\_design.md}.
