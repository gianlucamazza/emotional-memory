...
\newcommand{\zenodoversiondoi}{10.5281/zenodo.22724258}% [ssot:version_doi]
...
prediction-error term, validated by 127 fidelity cases across 20 phenomena.
Our central result is a
characterized \emph{boundary}: affect improves retrieval exactly when relevance is
itself affect-conditioned. Within that regime AFT reaches \texttt{top1\_accuracy}=0.53
vs.\ 0.33 for semantic-only cosine (v2, $N=200$, SBERT; $\Delta=+0.21$,
$p<0.001$, $d=0.49$), and a pre-registered audit shows ${\sim}62\%$ of it is affect-discriminative by construction---the advantage
...
human-validated on EmoBank (valence $r=0.79$), beating the theory-driven projection on all axes. (ii)~Appraising the query \emph{at retrieve time} recovers most of it
with no oracle ($\Delta=+0.115$, $p<0.001$; ${\sim}59\%$ recovery, ${\sim}82\%$ on the
affect-discriminative subset)---production-reachable, yet bounded: null on
naturalistic dialogue ($\Delta=-0.008$) and \emph{negative} on both released
third-party corpora ($\Delta=-0.083$, $-0.150$). The ranking edge converts end-to-end
(LLM-judged accuracy 0.595 vs.\ 0.440, $p<0.001$); with automatic appraisal on
...
with no retrieval-time LLM call. Released as the MIT-licensed
\texttt{emotional-memory} library.
\end{abstract}
...
\begin{sloppypar}
We survey 33 papers from 2022--2026; a full review is available in
...
in LLM agents, respectively.
\citet{subaharan2026dynamics} maintain an explicit external VAD state with first-
and second-order update dynamics (exponential smoothing and momentum) to control
long-horizon agent behavior---structurally parallel to AFT Layers~2--3, but with
no appraisal-based encoding, no memory store, and no retrieval pipeline.
On measurement, \citet{inoshita2026labels} benchmark zero-shot LLM VAD rating
against EmoBank distributions, and \citet{tak2025aware} document systematic
appraisal bias in LLMs (situations evaluated as if by a high-agreeableness,
low-power rater); \citet{fang2025personalization} show that persistent user
memory itself shifts LLM emotional reasoning along demographic lines---a caution
relevant to any affect-aware memory system, ours included.
None of these systems simultaneously implement the
...
$+0.15$ positive valence bias persists---consistent with the systematic
positivity-leaning appraisal bias independently documented in LLMs by
\citet{tak2025aware}; the rule-based \texttt{KeywordAppraisalEngine}
...
${\approx}0$), arousal $r=0.58$, dominance $r=0.43$ vs.\ $0.70/0.23/0.31$. For context,
these figures also exceed the zero-shot LLM ranges recently reported on the same corpus
(valence $r=0.49$--$0.67$, arousal $r=0.19$--$0.34$ across four models;
\citealp{inoshita2026labels}), though models and prompts differ, so we read this as
evidence that the schema-constrained rating protocol matters, not as a controlled
model comparison. This direct-VAD
...
(shrink-to-mean); the same low-MAE-via-compression trade-off is independently documented
by \citet{inoshita2026labels}, whose lowest-MAE model is also their lowest-$r$ one. The
durable claim is the dual-axis dominance via $r$-preservation, with the
...
A further pre-registered test (Addendum~X; closure:\footnote{%
\href{\repourl/blob/main/benchmarks/preregistration\_addendum\_x\_madialbench\_third\_party\_closure.md}%
{\texttt{preregistration\_addendum\_x\_madialbench\_third\_party\_closure.md}}.}) ran the same
oracle-free mechanism on the first released \emph{third-party} retrieval-native emotional
corpus (MADial-Bench EN \citep{he2025madialbench}: 160 emotion-triggered recall queries with
gold memory sets). Cosine won \emph{significantly} (nDCG@5 $0.304$ vs $0.221$, $\Delta=-0.083$
[$-0.123$, $-0.043$], $d=-0.317$; a powered negative, MDE $0.051$), even though the appraisal
is near-perfect against the third-party labels (AUC $=0.996$) and the corpus is \emph{more}
affect-discriminative than v2 ($76.9\%$ vs $62.5\%$). A post-hoc analysis explains why:
$84/160$ queries carry negative appraised valence, and for $73.8\%$ of them the gold memories
are \emph{positive}---the benchmark operationalizes emotion-triggered recall as interpersonal
emotion \emph{regulation} (recall a positive memory to support a distressed user), i.e.\
counter-congruent recall, the opposite of AFT's mood-congruence prior \citep{bower1981mood}.
The query-appraisal advantage is therefore bounded on three measured axes: \emph{regime}
(Addenda~U/T2A), \emph{provenance} (positive evidence only on author-crafted corpora so far),
and \emph{construct} (intrapersonal congruence vs.\ supportive counter-congruence). A
support-mode retrieval profile grounded in the emotion-regulation literature is the natural
theory-level follow-up.

The replication reserved by that closure (Addendum~X2; closure:\footnote{%
\href{\repourl/blob/main/benchmarks/preregistration\_addendum\_x2\_esmemeval\_third\_party\_closure.md}%
{\texttt{preregistration\_addendum\_x2\_esmemeval\_third\_party\_closure.md}}.}) ran the same
mechanism on the second released third-party corpus, ES-MemEval/EvoEmo
\citep{chen2026esmemeval}: longitudinal emotional-support QA ($N{=}1{,}133$ in-family
queries over 401 session documents, 50-candidate pools replicating the upstream
session-level protocol, upstream-verbatim metrics). Cosine won again, by twice the margin
(nDCG@4 $0.284$ vs $0.133$, $\Delta=-0.150$ [$-0.165$, $-0.136$], $d=-0.622$; MDE $0.018$),
with faithful appraisal (AUC $=0.950$) and---contrary to the pre-registered low-D2
prior---an affect-discriminative corpus ($63.0\%$). The post-hoc failure mode is
\emph{distinct}: the gold is not counter-congruent (corr(query, gold valence) $=+0.25$;
only $13.1\%$ opposite-sign) but \emph{affect-orthogonal}---$45.2\%$ of queries are
affectively neutral and the correct session is fixed by content, so the affect channel
buys ranking variance with no gold-directed signal. Two third-party corpora, two distinct
measured failure modes; what they share is that the gold relation was not authored around
mood-congruence. The operative boundary is thus sharper than the D2 regime criterion:
\emph{the gold relation itself must be affect-conditioned}, observed to date only in
author-crafted corpora.

A pre-registered query-affect gate (Addendum~Y; closure:\footnote{%
\href{\repourl/blob/main/benchmarks/preregistration\_addendum\_y\_query\_affect\_gate\_closure.md}%
{\texttt{preregistration\_addendum\_y\_query\_affect\_gate\_closure.md}}.}) routes
neutral queries ($\lvert\mathrm{valence}\rvert<0.2$) to pure cosine and affect-carrying
queries to the retrieve-time arm. Both primary hypotheses PASS (Holm $m=2$): the gate
recovers the neutral-query slice of the off-regime penalty on ES-MemEval
($\Delta=+0.076$ vs.\ always-on AFT) and preserves the on-regime gain on curated
($\Delta=+0.095$ vs.\ cosine). It does \emph{not} move the X/X2 boundary---affect-carrying
queries whose gold is counter-congruent or content-determined stay un-gateable. The
mechanism shipped as \texttt{retrieve\_query\_gated()} in v0.17.0.

A held-out linear learning-to-rank profile over the six retrieval signals
(Addendum~Z; closure:\footnote{%
\href{\repourl/blob/main/benchmarks/preregistration\_addendum\_z\_learned\_profile\_closure.md}%
{\texttt{preregistration\_addendum\_z\_learned\_profile\_closure.md}}.}) does
\emph{not} beat cosine on any third-party corpus (Hz1 FAIL, $0/3$; MADial
$\Delta=+0.015$, ES-MemEval $\Delta=-0.015$, DailyDialog $\Delta=+0.008$) while
preserving the curated on-regime advantage (Hz2 PASS, learned top1 $=0.425$ vs.\
fixed $0.240$). The boundary hardens from fixed-weight failure to: no held-out
learned linear affect profile generalizes on third-party gold.
...
(Addenda A--X2, plus the Hd2 power top-up) since initial submission. The expanded realistic-recall benchmark ($N=200$, five
...
to the affect-discriminative regime (Addendum~T2A, $\Delta=-0.008$), and both released
third-party tests are significantly negative---MADial-Bench (Addendum~X,
$\Delta=-0.083$; counter-congruent supportive recall) and ES-MemEval (Addendum~X2,
$\Delta=-0.150$; affect-orthogonal QA gold)---so positive retrieval evidence remains
confined to corpora whose gold relation is affect-conditioned by construction. Per-category
weight routing and query-type classification are closed negatives (Addenda~J/L).
State-based affect gating cannot exceed cosine (Addendum~Q). A query-affect-conditioned
gate is a validated safe wrapper and shipped as \texttt{retrieve\_query\_gated()}
(Addendum~Y, Branch~A); a held-out learned linear profile does not beat cosine on
third-party gold (Addendum~Z, Branch~B).
...
This work received no external funding and was conducted independently.
It builds on the open-source scientific Python ecosystem; we thank the
maintainers of the libraries on which \texttt{emotional-memory} depends, and
the authors of EmoBank, DailyDialog, LoCoMo, MADial-Bench, and ES-MemEval for releasing the
corpora used in the evidence programme.
