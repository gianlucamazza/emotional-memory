...
momentum tracking velocity and acceleration of emotional change, (3)~a slow-adapting background mood field (PAD exponential moving average), (4)~a Scherer appraisal vector, and (5)~a resonance
...
(e5-small-v2: $\Delta=+0.16$, $p<0.001$). This advantage holds under
oracle-provided affect labels; with \texttt{LLMAppraisalEngine} in dual-path
mode on affect-free data (Addendum~G, $N=200$), architecture parity is
maintained but the pre-registered minimum effect is not reached
(Hg1 FAIL, $\Delta=-0.010$, $d=-0.032$), scoping the main result to
preset-affect scenarios. The implementation is released
...
\texttt{gpt-4.1-mini} for both their internal LLM extraction and search
calls.\footnote{SOTA closure:
\href{\repourl/blob/main/benchmarks/comparative/protocol.sota.md}%
{\texttt{protocol.sota.md}};
results: \href{\repourl/blob/main/benchmarks/comparative/results.sota.sbert.md}%
{\texttt{results.sota.sbert.md}}.}
On the same four-quadrant probe, Mem0 achieves recall@5\,=\,1.00 [1.00, 1.00]
($\Delta=+0.20$ vs.\ cosine, paired bootstrap $p=0.045$, McNemar $p=0.125$);
LangMem 0.90 [0.75, 1.00] ($\Delta=+0.10$, $p_{\text{boot}}=0.254$).
On quality, AFT (0.85) is below both LLM-backed systems on this probe.
...
with no LLM dependency at runtime.
This positions AFT as a deterministic, low-latency alternative on
affect-aware retrieval probes where the LLM cost dimension matters; the
result does not establish AFT quality superiority on this dataset.
The hypothesis was registered as exploratory ($H_{\text{sota}}$, no Holm
correction). SOTA replication on the harder \texttt{realistic\_recall\_v2}
benchmark---where AFT shows the $\Delta=+0.21$ advantage over cosine---is
...
($H_{e2}$: $\Delta=+0.010$ on both embedders).
...
Expanded baselines were partially executed in this revision: Mem0 and LangMem
on \texttt{affect\_reference\_v1} (\S\ref{sec:benchmark} SOTA reference)
show that production LLM-backed memory dominates AFT on quality
(Mem0 1.00 vs.\ AFT 0.85) at $\sim$25$\times$ higher encode latency.
SOTA replication on \texttt{realistic\_recall\_v2} and on the LoCoMo factual
QA benchmark remains open work.
