...
slow-adapting PAD mood field, a Scherer appraisal vector, and a resonance graph with
spreading activation and Hebbian strengthening---and retrieves with a six-signal
mood-adaptive scorer gated by a Pearce-Hall prediction-error term. We validate it
through 127 fidelity cases over 20 psychological phenomena. On a
multi-session recall benchmark (v2, $N=200$), AFT reaches
\texttt{top1\_accuracy}=0.53 vs.\ 0.33 for semantic-only cosine ($\Delta=+0.21$
[0.15, 0.27], $p<0.001$, $d=0.49$; SBERT), replicated on e5-small-v2
($\Delta=+0.16$, $p<0.001$). The advantage holds under oracle-provided affect; with
automatic \texttt{LLMAppraisalEngine} appraisal on affect-free data it does not
(Addendum~P FAIL, $\Delta=-0.087$, $p=0.002$), and a pre-registered gating study
(Addendum~Q) locates the boundary at retrieve-time state injection rather than
appraisal calibration. Within the affect-discriminative (oracle-affect) regime the
ranking edge converts end-to-end: a pre-registered
encode$\to$retrieve$\to$generate$\to$judge task (Addendum~R, $N=200$) gives higher
LLM-judged answer accuracy than cosine (0.595 vs.\ 0.440, $\Delta=+0.155$,
$p<0.001$). Against human-annotated affect (Addendum~S, EmoBank, $N=300$), LLM
appraisal valence is human-validated ($r=0.70$); arousal and dominance correlate
only weakly. AFT encodes 25$\times$ faster than Mem0 (45 vs.\ 1130\,ms/item,
CPU-only) with no retrieval-time LLM call. The implementation is released as the
MIT-licensed \texttt{emotional-memory} Python library.
\end{abstract}
...
(Addenda A--S, plus the Hd2 power top-up) since initial submission. The expanded realistic-recall benchmark ($N=200$, five
