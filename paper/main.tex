...
The advantage extends to a multilingual cross-language slice
(Italian $N=80$, 20~scenarios $\times$ 4~queries, \texttt{multilingual-e5-small}):
$\Delta=+0.163$, $p=0.012$, $d=0.289$.\footnote{$H_{d2}$ closure:
...
benchmark; both pass the pre-registered $\Delta>0.10$ threshold and
generalize the architecture-attribution claim across embedder family
and language family.
...
and $\Delta=+0.163$ on Italian cross-language with \texttt{multilingual-e5-small}
($N=80$)).
...
the Italian proof-of-concept (G6); a dominance dimension as first-class
CoreAffect field (design in \texttt{docs/research/11\_dominance\_design.md});
and per-task retrieval weight tuning to investigate the LoCoMo scope limitation.
