...
\newcommand{\zenodoversiondoi}{10.5281/zenodo.20040352}% [ssot:version_doi]
...
future work targeting the LoCoMo gap requires architectural changes (e.g.\
per-category weight routing or query-type classification).\footnote{Pareto
...
The ablation finding for the resonance layer ($H_b$: $\Delta=+0.085$,
$p<0.001$ on e5-small-v2) is a confirmed \emph{magnitude amplification}, not
...
vs.\ SBERT $\Delta=+0.025$).\footnote{Addendum~I decomposition:
\href{\repourl/blob/main/benchmarks/preregistration\_addendum\_i.md}%
{\texttt{preregistration\_addendum\_i.md}}.}
Removing \texttt{ResonanceLink} \emph{improves} top1 accuracy for e5 in
aggregate; SBERT yields a non-significant $\Delta=+0.02$ ($p=0.203$).
The most plausible explanation is geometry-specific: e5-small-v2 clusters
semantically related items more tightly than SBERT-bge; spreading activation
over a tight-cluster geometry amplifies distractor promotion on challenge types
requiring affective discrimination among topic-near memories.
Mechanism confirmation (Hi3: link-set instrumentation, $N\geq500$) is deferred
to when a larger affect-rich dataset is available.
...
beyond the English/Italian/Spanish slices validated here; and architectural
approaches to LoCoMo factual QA (per-category weight routing, query-type
classification).
