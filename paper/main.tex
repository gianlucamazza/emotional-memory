...
recall benchmark, AFT achieves \texttt{top1\_accuracy}=0.70 vs.\ 0.50 for
semantic-only cosine retrieval (SBERT embedder). The implementation is released
...
realistic recall benchmark ($N=100$, top-1 retrieval over a non-trivial
candidate pool), where \texttt{top1\_accuracy}\,=\,0.70 vs.\
\texttt{naive\_cosine}\,=\,0.50 (SBERT; \texttt{benchmarks/realistic/results.sbert.md}).
...
The realistic-recall advantage ($\Delta=+0.20$, SBERT) is conditional on a
dense sentence embedder; under a non-semantic (hash) embedder the gap reduces
to $\Delta\approx+0.06$ and does not reach significance.
...
Pre-registered future work follows a staged evidence programme: an external
multi-session conversational QA benchmark (LoCoMo, Study S1), an expanded
realistic-recall dataset at $N\geq200$ with cross-embedder replication
(Study S2, Addendum B), and a human-evaluation pilot to ground-truth affective
coherence ratings (Addendum C). A dominance dimension is scoped for v0.8.0
following the design constraints in \texttt{docs/research/11\_dominance\_design.md}.
