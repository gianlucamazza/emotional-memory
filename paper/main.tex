...
\newcommand{\zenodoversiondoi}{10.5281/zenodo.20959964}% [ssot:version_doi]
...
through 127 fidelity cases over 20 psychological phenomena. On a multi-session recall
benchmark (v2, $N=200$, SBERT), AFT reaches \texttt{top1\_accuracy}=0.53 vs.\ 0.33 for
semantic-only cosine ($\Delta=+0.21$ [0.15, 0.27], $p<0.001$, $d=0.49$; replicated on
e5-small-v2). A pre-registered circularity audit bounds this: ${\sim}62\%$ of queries
are affect-discriminative by construction and the advantage concentrates there (null on
the rest). With automatic appraisal on affect-free data the advantage disappears
($\Delta=-0.087$, $p=0.002$), locating the boundary at retrieve-time state injection.
Crucially, appraising the query \emph{at retrieve time} (no oracle) recovers most of
it---AFT still beats cosine ($\Delta=+0.115$, $p<0.001$; ${\sim}59\%$ recovery,
${\sim}82\%$ on the affect-discriminative subset)---so the advantage is largely
production-reachable. The ranking edge also converts end-to-end (LLM-judged answer
accuracy 0.595 vs.\ 0.440, $p<0.001$), and direct-LLM valence is human-validated against
EmoBank ($r=0.79$). AFT encodes 25$\times$ faster than Mem0 (45 vs.\ 1130\,ms/item,
...
oracle is not required---though this remains bounded to that regime; query appraisal on
naturalistic, affect-sparse QA (LoCoMo/DailyDialog) is untested.
...
(Addenda A--V, plus the Hd2 power top-up) since initial submission. The expanded realistic-recall benchmark ($N=200$, five
