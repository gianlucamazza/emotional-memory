...
class TestMoodField:
    ...
    def test_clamp_valence(self):
        s = MoodField(
            ...
            dominance=0.5,
            inertia=0.5,
            timestamp=MoodField.neutral().timestamp,
        )
...
class TestMoodRegress:
    def test_zero_elapsed_returns_identical_values(self):
        ...
        r = s.regress(s.timestamp, MoodDecayConfig())
    ...
    def test_regression_moves_toward_baseline(self):
        ...
        cfg = MoodDecayConfig(base_half_life_seconds=1.0, inertia_scale=0.0)
        s = MoodField(
    ...
    def test_half_life_semantics(self):
        ...
        half_life = 3600.0
        cfg = MoodDecayConfig(base_half_life_seconds=half_life, inertia_scale=0.0)
        s = MoodField(
    ...
    def test_high_inertia_slows_regression(self):
        ...
        cfg = MoodDecayConfig(base_half_life_seconds=1.0, inertia_scale=10.0)
        ts = datetime.now(tz=UTC)
        rigid = MoodField(valence=1.0, arousal=0.5, dominance=0.5, inertia=0.9, timestamp=ts)
        fluid = MoodField(valence=1.0, arousal=0.5, dominance=0.5, inertia=0.0, timestamp=ts)
    ...
    def test_regress_returns_new_instance(self):
        ...
        r = s.regress(future, MoodDecayConfig())
        assert r is not s

    def test_update_with_decay_config_regresses_first(self):
        """Mood after a long silence should reflect baseline pull."""
        from datetime import UTC, datetime, timedelta
        ...
        cfg = MoodDecayConfig(base_half_life_seconds=1.0, inertia_scale=0.0)
        ts = datetime.now(tz=UTC)
        s = MoodField(valence=1.0, arousal=0.9, dominance=0.9, inertia=0.0, timestamp=ts)
    ...
    def test_distance_symmetric(self):
        a = MoodField.neutral()
        b = MoodField(valence=0.5, arousal=0.5, dominance=0.8, inertia=0.3, timestamp=a.timestamp)
...
class TestMoodProperties:
    """Mathematical invariants for MoodField."""
    ...
    def test_bounded_after_extreme_updates(self, valence, arousal):
        """After many extreme-valued updates, Mood stays in valid ranges."""
        s = MoodField.neutral()
    ...
    def test_values_always_in_range(self, n_updates):
        """Mood stays in bounds regardless of update count."""
        s = MoodField.neutral()
